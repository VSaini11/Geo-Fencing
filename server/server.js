...
// Update live location for logged-in user
...
app.post('/api/checkins', authenticateToken, async (req, res) => {
  try {
    ...
    // TRIGGER PUSH NOTIFICATION (EMPLOYEE)
    ...
    // TRIGGER PUSH NOTIFICATION (ADMINS)
    ...
    for (const admin of admins) {
      if (admin.pushToken) {
        sendPushNotification(
          admin.pushToken,
          "Employee Arrived ✅",
          `${req.user.name} has successfully checked in at ${loc ? loc.name : 'the job site'}.`
        );
      }
    }
...
app.post('/api/checkins/:id/ping', authenticateToken, async (req, res) => {
  try {
    ...
    // Store in DailyLog for location history (if needed)
    const dateStr = new Date().toISOString().split('T')[0];
    const dailyLog = await DailyLog.findOne({ userName: checkIn.userName, date: dateStr });
    if (dailyLog) {
      // Throttle: Only add a ping event to DailyLog history if the last one was > 4.5 minutes ago
      // to avoid bloating the database with the foreground's 5-second pings.
      const lastPingEvent = [...dailyLog.events].reverse().find(e => e.type === 'ping');
      const now = Date.now();
      if (!lastPingEvent || (now - new Date(lastPingEvent.time).getTime() > 270000)) {
        dailyLog.events.push({
          time: new Date(),
          type: 'ping',
          coords: { latitude, longitude }
...
app.post('/api/checkins/:id/checkout', authenticateToken, async (req, res) => {
  try {
    ...
    if (dailyLog) {
      ...
      // Short Hours Half Day tagging based on < 5 hours (300 minutes)
      if (dailyLog.workedMinutes < 300) {
    ...
    for (const admin of admins) {
      if (admin.pushToken) {
        sendPushNotification(
          admin.pushToken,
          "Employee Out of Range ⚠️",
          `${checkIn.userName} has gone out of range from ${loc ? loc.name : 'the job site'}.`
        );
      }
    }
